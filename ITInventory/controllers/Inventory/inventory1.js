...
import('node-fetch').then(fetchModule => {
    ...
    // Rest of your routes and logic here...
...
// Endpoint to gather and send client information with explicit user consent
app.get('/captureInfo', async (req, res) => {
    try {
        // Inform the client about data collection and ask for consent
        res.render('consentPage'); // Render a consent page using your view engine

    } catch (error) {
        console.error('Error rendering consent page:', error);
        res.status(500).send('Error rendering consent page.');
    }
});
// Function to gather client information
async function gatherClientInfo(req) {
    try {
...
// Endpoint to handle user consent and gather/send client information
app.post('/captureInfo', async (req, res) => {
    try {
        const consent = req.body.consent; // Assuming consent is submitted in the request body

        if (consent !== 'yes') {
            return res.status(400).send('Consent not given.'); // User declined consent
        }

        // Proceed to gather and send client information
        const clientInfo = await gatherClientInfo(); // Function to gather client information

        // Send the information to your server
        const serverEndpoint = 'http://172.16.21.91:8080/storeInfo';
