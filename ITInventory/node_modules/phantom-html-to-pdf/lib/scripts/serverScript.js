...
page.onResourceRequested = function (request, networkRequest) {
    ...
    if (request.url.lastIndexOf("file://", 0) === 0 && request.url.lastIndexOf("file:///", 0) !== 0) {
     ...
     if (request.url.lastIndexOf("http://", 0) !== 0 && 
        request.url.lastIndexOf("https://", 0) && 
        request.url.lastIndexOf("data:", 0) && 
        !body.allowLocalFilesAccess) {
...
page.open(body.url, function () {
    ...
    function resolvePage() {
        ...
        if (body.waitForJS) {
            phantomHeader = page.evaluate(function (s) {
            return document.querySelector(s) ? document.querySelector(s).innerHTML : null;
            }, '#phantomHeader');

            phantomFooter = page.evaluate(function (s) {
                return document.querySelector(s) ? document.querySelector(s).innerHTML : null;
            }, '#phantomFooter');
        }
